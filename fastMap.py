try:
    ...
    from scripts.installModules import *
    from scripts.modules import *
...
def check():
    global ip
    f = open(os.path.join('log', 'json_data.json'), 'r')
    read = str(f.read())
    f.close()
        ...
        if file == None:
            ...
            while True:
                try:
                    ...
                    sys.exit(f'{gordo}{rojo}ERROR{reset} : The indicated file does not exist, make sure you have spelled the name correctly.')
                for i in read:
                    ip = i.rstrip()
                    f = open(os.path.join('log', 'json_data.json'), 'r')
                    read = str(f.read())
